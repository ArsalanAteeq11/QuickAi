...
const BlogTitles = () => {
  ...
  const [selectedCategoy, setSelectedCategory] = useState("General");
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      setLoading(true);
      const prompt = `Genearte a blog title for the keyword ${input} in the category ${selectedCategoy}`;
        ...
        <div className="mt-3 flex gap-3 flex-wrap sm:max-w-9/11">
          {blogCategories.map((item, index) => (
            <span
              ...
              className={`text-xs px-4 py-1 w-30 text-center
          border rounded-full
          cursor-pointer ${
